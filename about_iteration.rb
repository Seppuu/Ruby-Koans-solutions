...
class AboutIteration < Neo::Koan
  ...
  def test_inject_will_blow_your_mind
    ...
    #inject 带有一个参数和一个block.block中有两个参数,第一个参数是传入的参数(如果没传,就是数组中的第一个值),第二个参数就是数组中的元素.该block一共执行3次,每次执行之后,都将返回值赋给result,之后result再被传入block中的作为第一个参数,继续执行.直到遍历完所有数组中的元素.
  end
